...
import static org.kse.crypto.keypair.KeyPairType.ECGOST3410_2012_256;
...
public final class KeyPairUtil {
    ...
    public static KeyPair generateKeyPair(KeyPairType keyPairType, int keySize, Provider provider) throws CryptoException {
        try {
            ...
            if (provider != null) {
                ...
                if (keyPairType == RSA || keyPairType == ECGOST3410_2012_256) {
    ...
    public static boolean validKeyPair(PrivateKey privateKey, PublicKey publicKey) throws CryptoException {
        try {
            ...
            if (privateAlgorithm.equals(RSA.jce())) {
                ...
                return verify(toSign, signature, publicKey, signatureAlgorithm);
            } else if (privateAlgorithm.equals(ECGOST3410_2012_256.jce())) {
                byte[] toSign = "ECGOST3410_2012_256".getBytes();
                String signatureAlgorithm = "GOST3411-2012-256WITHECGOST3410-2012-256";
                byte[] signature = sign(toSign, privateKey, signatureAlgorithm);
    ...
    private static boolean verify(byte[] signed, byte[] signaureToVerify, PublicKey publicKey, String
            signatureAlgorithm)
            throws GeneralSecurityException {
        Signature signature = Signature.getInstance(signatureAlgorithm, new BouncyCastleProvider());
    ...
    public static KeyPair generateGostKeyPair(String curveName, Provider provider) throws CryptoException {
        try {
            // Get a key pair generator
            KeyPairGenerator keyPairGen;
            if (provider != null) {
                keyPairGen = KeyPairGenerator.getInstance(ECGOST3410_2012_256.jce(), provider);
            } else {
                keyPairGen = KeyPairGenerator.getInstance(KeyPairType.ECGOST3410_2012_256.jce(), BOUNCY_CASTLE.jce());
            }
            ...
            throw new CryptoException(MessageFormat.format(res.getString("NoGenerateKeypair.exception.message"),
                    KeyPairType.ECGOST3410_2012_256), ex);
        }
    }
}
